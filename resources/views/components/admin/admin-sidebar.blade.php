<aside class="hidden lg:block">
       <div class="sticky top-8 space-y-8">
           ...
           <nav aria-label="Admin navigation" class="space-y-2">
               <a href="{{route('admin.dashboard')}}#overview" @class([
                   'flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition-colors',
                   'bg-orange-500 font-semibold text-white shadow-lg shadow-orange-500/10' => request()->routeIs('admin.dashboard'),
                   'font-medium text-gray-400 hover:bg-gray-900 hover:text-white' => ! request()->routeIs('admin.dashboard'),
               ])><i class="fa-solid fa-chart-line w-4 text-center"></i>Overview</a>
               <a href="{{route('admin.reports')}}" @class([
                   'flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition-colors',
                   'bg-orange-500 font-semibold text-white shadow-lg shadow-orange-500/10' => request()->routeIs('admin.reports'),
                   'font-medium text-gray-400 hover:bg-gray-900 hover:text-white' => ! request()->routeIs('admin.reports'),
               ])><i class="fa-solid fa-chart-pie w-4 text-center"></i>Reports &amp; Analytics</a>
               <a href="{{route('all.orders')}}" class="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-gray-400 transition-colors hover:bg-gray-900 hover:text-white"><i class="fa-solid fa-receipt w-4 text-center"></i>Orders <span class="ml-auto rounded-full bg-orange-500/15 px-2 py-0.5 text-xs text-orange-300">{{$totalOrders}}</span></a>
